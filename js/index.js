...
//   card.innerHTML = `
//     <h3 class="heading-count-icon"><span class="home-icon"></span>Aging Status</h3>
//     <div class="leaderboard-chart">${
//       leaderboardHTML || "<p>No votes yet.</p>"
//     }</div>
//   `;
...
function showAgingStatus() {
  const IDEAS_LS_KEY = "ideas"; // Replace with your localStorage key
  const ideas = JSON.parse(localStorage.getItem(IDEAS_LS_KEY)) || [];
  ...
  const agingHTML = ideas
    .map((idea) => {
      const submissionMonthIndex = monthMap[idea.month] ?? -1;
      if (submissionMonthIndex === -1) return null;
      const delay = currentMonthIndex - submissionMonthIndex;
      if (delay <= 0) return null;

      return `
    <div class="aging-entry" style="margin-bottom: 30px;">
      <strong>${idea.userName}</strong>
      <br/><small>Aging: ${delay} month${delay > 1 ? "s" : ""} delayed</small>
    </div>
  `;
    })
    .filter(Boolean)
    .join("");
  // card.innerHTML = <h3 class="heading-count-icon"><span class="home-icon"></span>Aging Status</h3> <div class="leaderboard-chart"> ${agingHTML || "<p>No delayed ideas found.</p>"} </div> ;

  card.innerHTML = `
  <h3 class="heading-count-icon"><span class="home-icon"></span>Aging Status</h3>
  <div class="leaderboard-chart">${
    agingHTML || "<p>No delayed ideas found.</p>"
  }</div>
`;
}

function addDataButtonVisibility() {
  const value = JSON.parse(localStorage.getItem("sampleData"));
  console.log("value", value);
  const dataButton = document.getElementById("add-sample-data");
  if (!value) {

function renderCount(ideas) {
    ...
    if (Array.isArray(ideas)) {
        ...
        totalOperationIdeas = ideas.filter(idea => String(idea.category).trim().toLowerCase() === "transport").length;
    }
...
function renderIdeas(ideas) {
    ...
    if (Array.isArray(ideas)) {
        ideas.filter(el => String(el.category).toLowerCase() === "transport").forEach(idea => {

function renderCount(ideas) {
    ...
    if (Array.isArray(ideas)) {
        ...
        totalOperationIdeas = ideas.filter(idea => String(idea.category).trim().toLowerCase() === "technology").length;
    }
...
function renderIdeas(ideas) {
    ...
    if (Array.isArray(ideas)) {
        ideas.filter(el => String(el.category).toLowerCase() === "technology").forEach(idea => {
